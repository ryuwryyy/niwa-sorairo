---
name: mono-color
description: Generate original one-ink or controlled two-ink editorial images from any theme, sentence, article idea, object, or reference photo. Always use this skill when the user asks for 单色海报、双色印刷、单色调视觉、蓝色/绿色孔版印刷、risograph、网点照片、复古或当代编辑排版、zine poster, monochrome editorial poster, duotone print, or asks to use the mono-color style. Also use it for Japanese requests such as 単色ポスター、二色刷り、モノカラー、リソグラフ風、網点写真、孔版印刷風、ジン／ZINEの表紙、エディトリアルなビジュアル, and for key visuals, covers, or デコンテ frames in this repo's design tasks. It uses an adaptive white, gray, or pale-beige substrate, no more than two printing inks, active negative space, terse human language, and strong serif/grotesk/mono typography without making retro styling the default or copying a source composition, wording, logo, or artwork. Produce both the final generation prompt and the generated raster image unless the user explicitly asks for prompt only.
---

# Monocolor Editorial Print

> Vendored from [yanliudesign/mono-color-skill](https://github.com/yanliudesign/mono-color-skill) (commit `c8ff705`, MIT — see `LICENSE`). The upstream `examples/` images are NOT included: they are under a separate all-rights-reserved asset license and third-party copyright. Only the skill text, `design-system/` catalogs, `swatches/`, and `evals/` are copied.

## このリポジトリでの使い方(上流からの変更点)

- **出力の置き場所:** 生成物はセッションの scratchpad か、持ち主が指定した場所に置く。リポジトリに入れるのは頼まれたときだけ(`research/` や `public/` に勝手に画像を足さない)。
- **画像生成:** 画像生成ツールが使えるとき(例: Figma の Weave `weave_run_model`)は、課金が起きうるので初回だけ持ち主に確認してから生成する。使えないときは「最終プロンプト+配方メモ」を出し、必要なら `design-system/colors.json` の色・`typography.json` の書体役割に従って SVG/HTML で版下(文字と構図の叩き台)を組む。
- **言葉:** 持ち主が日本語で書いた文言はそのまま日本語で使う。文言がないときの既定は上流どおり英語の短い一句。
- **出力の見出し:** 下の Output Format の中国語見出しは日本語に置き換える(**生成画像** / **最終プロンプト** / **今回の配方**)。
- **UI には使わない:** アプリの画面デザインは `design-md-sources`。このスキルはポスター・表紙・キービジュアル・デコンテのコマ・FigJam に貼る一枚絵など「印刷物っぽい一枚」のためのもの。`social-deepdive` のデコンテの絵作りに使ってよい。
...
Save generated files where the repo-usage notes above say (scratchpad by default). Stop at prompt-only only when the user explicitly asks for it or no image-generation capability is available.
