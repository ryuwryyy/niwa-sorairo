...
4. このリポジトリの `studio-figma-plugin/manifest.json` を選ぶ。
...
```bash
node studio-figma-plugin/samples/make-png.mjs              # kv.png を作り直す
node studio-figma-plugin/samples/make-png.mjs --patch-spec # sample-spec.json の dataUrl も入れ替える
```
...
```bash
node --check studio-figma-plugin/code.js                 # 構文チェック
